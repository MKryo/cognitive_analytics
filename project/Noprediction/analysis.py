...
# データ読み込み
df_est = pd.read_csv('estimations_cre.csv')
df_pred = pd.read_csv('predictions_cre.csv')
...
for i in range(6):
    df_est_mouse_each[i] = df_est_mouse[df_est_mouse['est_i'] == i]
...
for i in range(6):
    df_est_rabbit_each[i] = df_est_rabbit[df_est_rabbit['est_i'] == i]
...
for i in range(6):
    df_est_pigeon_each[i] = df_est_pigeon[df_est_pigeon['est_i'] == i]
...
print('rabbit ΔP r2_score:', r2_score(df_rabbit['human'], df_rabbit['ΔP']))
...
print('pigeon ΔP r2_score:', r2_score(df_pigeon['human'], df_pigeon['ΔP']))
